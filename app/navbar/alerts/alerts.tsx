...
export default function Alerts({
    ...
    const { alerts, setAlerts } = useSSE();

    const [oldAlerts, setOldAlerts] = useState<AppAlert[]>([]);
    ...
    return (
        <div ref={notifications} style={{ marginTop: '-150vh' }} id="notifications" className={"text-slate-200 transition-all   min-h-96 h-96 max-w-[calc(25rem)] w-[calc(90svw)] bg-slate-800  drop-shadow-lg shadow-gray-900 absolute md:right-[calc(19rem)]  left-1/2 -translate-x-1/2 md:left-auto md:-translate-x-0 z-10 duration-500 self-start rounded-lg border border-slate-900"}>
            ...
            {current=='notification' &&  <Notifications notificationsAlerts={(alerts.filter(alert => 'post_id' in alert)) as NotificationAlert[]}  /> }
            {current=='requests' &&  <Requests requestsAlerts={alerts.filter((alert) => 'fullname' in alert) as ConnectRequestAlert[]} /> }
        </div>
    )
}